...
def finetuneing(dataset_info, finetuned_model_info):
    import json
    ...
    login(token=hutoken)
    print("logging in sucessful")
    # Defining the configuration for the base model, LoRA and training
    config = {
        "hugging_face_username":"Anony0324",
        "model_config": {
            "base_model":"unsloth/llama-3-8b-Instruct-bnb-4bit", # The base model
            "finetuned_model":finetuned_model_info, # The finetuned model
            "max_seq_length": 3000, # The maximum sequence length
            "dtype":torch.float16, # The data type
            "load_in_4bit": True, # Load the model in 4-bit
        },
        "lora_config": {
        "r": 16, # The number of LoRA layers 8, 16, 32, 64
        "target_modules": ["q_proj", "k_proj", "v_pfroj", "o_proj",
        ...
        "training_dataset":{
            "name":dataset_info, # The dataset name(huggingface/datasets)
    ...
    inputs = tokenizer(
    [
        "<|start_header_id|>system<|end_header_id|> Answer the question truthfully.<|eot_id|><|start_header_id|>user<|end_header_id|> This is the question: Do you know about samsung galaxy<|eot_id|>"
